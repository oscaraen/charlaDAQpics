...
float encontrarT(unsigned int registro){
    float Vin = (float)registro * 5.0f / 1024.0f;
    Vin = Vin / 2.94587f;
    float Rs = 100.0f * (Vin + 7.5f) / (7.5f - Vin);
    float Temp = (Rs - 100.0f) / 0.39f;
    return Temp;
}
float voltajef(unsigned int registro){
        float Vin = (float)registro * 5.0f / 1023.0f;
    return Vin;
}
...
void main(){
    ...
    putsUART("Saludo!!\r\n");
    float voltaje = 0, vin=0;
    while(1){
        ADCON0bits.GO = 1;
        while(ADCON0bits.GO);
        valADC = (ADRESH << 8) + ADRESL;
        voltaje = encontrarT(valADC);
        vin= voltajef(valADC);
        sprintf(mensaje,"temp= %.5f v= %.5f \r\n",voltaje, vin);
        putsUART(mensaje);
